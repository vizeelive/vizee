...
async function checkout_session_completed(event) {
  ...
  try {
    ...
    if (user && product.account_access) {
      access = user.access.find((rec) => rec.account_id === product.account_id);
    } else if (user) {
    ...
    const paymentIntent = await stripe.getPaymentIntent(session.payment_intent);
    ...
    let object = {
      ...
      ...(product.recurring ? { subscription: true } : { subscription: false }),
...
module.exports = checkout_session_completed;
