...
SELECT users.id AS user_id,
    ...
    data,
    ((((((stripe_subscriptions.data -> 'items'::text) -> 'data'::text) -> 0) -> 'plan'::text) -> 'interval_count'::text)::integer) AS interval_count,
    ((((((stripe_subscriptions.data -> 'items'::text) -> 'data'::text) -> 0) -> 'plan'::text) -> 'amount'::text)::integer / 100) * (((((((stripe_subscriptions.data -> 'items'::text) -> 'data'::text) -> 0) -> 'plan'::text) -> 'interval_count'::text)::integer) / 30.416)::money AS monthly_amount
