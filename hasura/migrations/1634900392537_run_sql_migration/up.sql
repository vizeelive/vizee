...
SELECT
	...
	(encoding_cost + s3_egress_cost + transloadit_ingest_cost) * 100 / 20 AS ingest_recovery_revenue,
	(s3_storage_cost_monthly + mux_storage_cost_monthly) * 100 / 20 AS monthly_recovery_revenue
FROM (
	SELECT
		accounts.id,
		accounts.name,
