...
SELECT
	...
	(encoding_cost + s3_egress_cost + transloadit_ingest_cost) * 100 / 4 AS ingest_recovery_revenue,
	(s3_storage_cost_monthly + mux_storage_cost_monthly) * 100 / 4 As monthly_recovery_revenue
FROM (
	SELECT
		accounts.id,
		accounts.name,
