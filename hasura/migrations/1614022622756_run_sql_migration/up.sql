CREATE OR REPLACE VIEW account_report AS
 SELECT
	...
	(SELECT COUNT(*) FROM views WHERE account_id = accounts.id) AS viewcount,
...
FROM
	...
	LEFT JOIN (
		...
		FROM
			subscriptions
		GROUP BY
			subscriptions.account_id) subscriptions_count ON (subscriptions_count.account_id = accounts.id)
	LEFT JOIN (
		SELECT
			events.account_id,
			count(*) AS events
		FROM
			events
		GROUP BY
			events.account_id) events_count ON events_count.account_id = accounts.id;
