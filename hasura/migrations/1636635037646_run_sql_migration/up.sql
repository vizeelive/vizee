CREATE OR REPLACE VIEW images AS
  SELECT posts_attachments.id,
    ...
    posts_attachments.width,
    posts_attachments.height
   FROM posts_attachments
     ...
     LEFT JOIN accounts event_account ON event_account.id = events.account_id
  WHERE posts_attachments.type = 'image'::text;
